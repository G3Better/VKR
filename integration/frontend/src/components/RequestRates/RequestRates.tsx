...
import {
  ...
  editRequestRates,
  getRequestRates
} from "../../controllers/RequestRatesController";
import { dateConverter } from "../../utills/dateUtills";
import Header from "../Header/Header";
import styles from "./RequestRates.module.sass";
import {checkIsArrayDataFromModal, uniqArrayForModal} from "../../utills/dataUtil";

const columns: GridColDef[] = [
  { field: "name", headerName: "Частота запроса", type: "string" },
];

const RequestRates: React.FC = () => {
  const [data, setData] = React.useState([]);
  const [open, setOpen] = React.useState(false);
  const [id, setId] = React.useState<string | undefined>(undefined);
  const [editData, setEditData] = React.useState<any>(null);
  const fetchData = React.useCallback(async () => {
    const dataTable = await getRequestRates();
    if (dataTable.length) {
      setData(dataTable);
    } else {
      setData([]);
  ...
  const handleDelete = React.useCallback(async () => {
    if (id) {
      const data = await deleteRequestRates(id);
      await fetchData();
  ...
  return (
      <>
        <Header />
        <h2 className={styles.systems_title}>Request Rates</h2>
        <TableData
            columns={columns}
            openModal={open}
            data={editData}
