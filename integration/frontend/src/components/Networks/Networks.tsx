...
import styles from "./Networks.module.sass";
import {checkIsArrayDataFromModal, uniqArrayForModal} from "../../utills/dataUtil";

const columns: GridColDef[] = [
  { field: "name", headerName: "Сеть", type: "string" },
];
...
const Networks: React.FC = () => {
  ...
  const fetchData = React.useCallback(async () => {
    const dataTable = await getNetworks();
    if (dataTable.length) {
      setData(dataTable);
    } else {
      setData([]);
  ...
  const handleDelete = React.useCallback(async () => {
    if (id) {
      const data = await deleteNetworks(id);
      await fetchData();
  ...
  return (
      <>
        <Header />
        <h2 className={styles.systems_title}>Networks</h2>
        <TableData
            columns={columns}
            openModal={open}
            data={editData}
